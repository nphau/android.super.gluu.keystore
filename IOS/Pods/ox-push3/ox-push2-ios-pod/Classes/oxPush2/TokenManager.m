...
-(void)enroll:(NSDictionary*)parameters baseUrl:(NSString*)baseUrl isDecline:(BOOL)isDecline isSecureClick:(BOOL)isSecureClick callBack:(TokenResponseCompletionHandler)handler {
    ...
    [u2FKey registerRequest:[[EnrollmentRequest alloc] initWithVersion:version challenge:challenge application:appParam issuer:baseUrl] isDecline:isDecline isSecureClick: isSecureClick callback:^(EnrollmentResponse *response, NSError *error){
        TokenResponse* tokenResponse = [self makeTokenResponse:response isDecline:isDecline challenge:challenge baseUrl:baseUrl];
...
-(TokenResponse*)sign:(NSDictionary*)parameters baseUrl:(NSString*)baseUrl isDecline:(BOOL)isDecline isSecureClick:(BOOL)isSecureClick callBack:(AuthenticateResponseeCompletionHandler)handler{
    ...
    for (NSDictionary* autenticateRequest in autenticateRequests){
        ...
        [u2FKey autenticate:[[AuthenticateRequest alloc] initWithVersion:version control:controlData challenge:challenge application:appParam keyHandle:keyHandle] isSecureClick:isSecureClick callback: ^(AuthenticateResponse *response, NSError *error){
            TokenResponse* authenticateResponse = [self makeAuthenticationResponse:response authenticatedChallenge:challenge isDecline:isDecline authRequest:authRequest baseUrl:baseUrl];
        }];
    }
    
}

-(TokenResponse*)makeTokenResponse:(EnrollmentResponse*)enrollmentResponse isDecline:(BOOL)isDecline challenge:(NSString*) challenge baseUrl:(NSString*)baseUrl{
